...
if ($_SERVER['REQUEST_METHOD'] === 'POST' AND isset($_POST['azione'])) {
    ...
    if ($_POST['azione'] === 'Aggiungi al carrello' && $id && $nome && $prezzo) {
        if($totQuantita+1<=10){
            ...
            $_SESSION['aggiunta_carrello'] = "<div><p class=\"messaggio\">Prodotto aggiunto al carrello! Continua a visitare il nostro menù e procedi all'ordine!</p></div>";
        }else {
            $_SESSION['aggiunta_carrello'] = "<div><p class=\"messaggio\">Ci spiace ma hai raggiunto la quantità massima di 10pizze. Contattaci telefonicamente per ordini maggiori</p></div>";
        }
